...
class VisitService:
    ...
    def find_free_slots(self, doctor_id, date, slot_minutes=30):
        ...
        intervals = []
        for s in schedule:
            start = datetime.strptime(s["day"] + " " + s["time_from"], "%Y-%m-%d %H:%M:%S")
            end = datetime.strptime(s["day"] + " " + s["time_to"], "%Y-%m-%d %H:%M:%S")
        ...
        for work_start, work_end in intervals:
            current = work_start
            busy_sorted = sorted([b for b in busy if b[0] < work_end and b[1] > work_start], key=lambda x: x[0])
